...
export default async function EditRoundPage({
  ...
  if (competitionError || !competition || roundError || !round) {
    notFound();
  }

  // Verify round belongs to competition
  if (round.competition_id !== id) {
    notFound();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <p className="text-gray-600">
            {t("round.editDescription")} - {competition.name}
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("round.basicInfo")}</CardTitle>
        </CardHeader>
        <CardContent>
          <RoundForm locale={locale} competitionId={id} round={round} />
        </CardContent>
      </Card>
    </div>
  );
